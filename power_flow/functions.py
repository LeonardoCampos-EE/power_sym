...
def calculate_hydro_goal_deviation(
    ...
    args:
        - P_g: jax array of shape (N, 1, T) that contains the power generated by the
        N hydro plants on T periods
...
def objective_function(Pg: jnp.DeviceArray, extra_variables: dict) -> float:
...
def restrictions_function(
    Pg: jnp.DeviceArray, extra_variables: dict
) -> jnp.DeviceArray:
    ...
    Delta_P = active_power_balance(P=P, P_g=Pg, P_c=extra_variables["Pc"])

    Q = reactive_power_flow(
        V=extra_variables["V"],
        theta=extra_variables["theta"],
        G=extra_variables["G"],
        B=extra_variables["B"],
        Bsh=extra_variables["Bsh"],
    )
    Delta_Q = reactive_power_balance(
        Q=Q, Q_g=extra_variables["Qg"], Q_c=extra_variables["Qc"]
    )

    hydro_goal = calculate_hydro_goal_deviation(P_g=Pg, goal=extra_variables["goal"])

    res = jnp.asarray(
        [
            [Delta_P.sum().astype(float)],
            [Delta_Q.sum().astype(float)],
            [hydro_goal.sum().astype(float)],
        ]
    )
